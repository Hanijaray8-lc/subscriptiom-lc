...
const PaymentTracking = () => {
  ...
  const fetchInvoices = async () => {
    ...
    try {
      const res = await axios.get(
       `https://subscriptiom-lc.onrender.com/api/amcinvoices?owner=${user._id}`
      );
  ...
  const filteredInvoices = invoices.filter((inv) => {
    ...
    // name filter
    if (searchName && !inv.customerName.toLowerCase().includes(searchName.toLowerCase())) {
      return false;
    }

    // product filter
    if (searchProduct) {
      const productMatch = inv.products.some(p =>
  ...
  const calculateTotal = (list) =>
    list.reduce((sum, inv) => sum + (inv.totalAmount || 0), 0);

  // 🔹 Show invoices for a single customer
  const handleShowCustomer = (custName) => {
    const custInvoices = invoices.filter((inv) => inv.customerName === custName);
  ...
  const exportToExcel = () => {
    const data = filteredInvoices.map((inv, i) => ({
      ...
      Product: inv.products.map(p => p.productName).join(", "),
            ...
            <tbody className="text-center">
              {filteredInvoices.map((inv, i) => (
                <tr
                  ...
                  onClick={() => handleShowCustomer(inv.customerName)}
                >
                  <td>{i + 1}</td>
                  <td>{inv.invoiceNumber}</td>
                  <td>{new Date(inv.date).toLocaleDateString()}</td>
                  <td className="fw-bold text-primary">{inv.customerName}</td>
                  <td>
                  ...
                  <td className="fw-bold text-success">₹{inv.totalAmount}</td>
                  <td>
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleShowCustomer(inv.customerName);
                      }}
                    >
                      View
                    </Button>
            ...
            <tbody className="text-center">
              {customerInvoices.map((inv, idx) => (
                <tr key={inv._id}>
                  ...
                  <td>{inv.invoiceNumber}</td>
                  <td>{new Date(inv.date).toLocaleDateString()}</td>
                  <td>{inv.products.map(p => p.productName).join(", ")}</td>
                  <td>{inv.subscription}</td>
              ...
              <tr className="fw-bold bg-light">
                ...
                <td className="text-success">₹{calculateTotal(customerInvoices)}</td>
              </tr>
            </tbody>
          </Table>
        </Modal.Body>
